...
const updateClient = async (id, email, contraseña, nombre, apellido, ciudad, direccion, telefono, edad, dni, imagen) => {
  const clientById = await Client.findByPk(id);

  if (!clientById) throw new Error({ error: `No customer was found with the ID ${id}. ` });
